...
export function RefundConfirmPage() {
  ...
  const operationQuery = useAsyncData(
    () => api.getRefundOperation(accessToken ?? '', operationId),
    ...
    return (
      <AppShell>
        <section className="rounded-[28px] border border-white/70 bg-white p-8 text-base text-slate-500 shadow-card shadow-slate-900/5">
          Загружаем подтверждение возврата...
        </section>
      </AppShell>
    );
  }
  ...
  if ((operationQuery.error && !resolvedOperation) || !resolvedOperation) {
    return (
      <AppShell>
        <ErrorState
          description={operationQuery.error ?? 'Не удалось загрузить расчёт возврата'}
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        <PageHeader
          backHref={routes.refund(orderId)}
          backLabel="Назад к расчёту"
          eyebrow={`Подтверждение · ${resolvedOperation.pnr}`}
          title="Подтверждение возврата"
          subtitle="Проверьте условия операции. После подтверждения возврат будет применён к заказу."
        />

        <QuoteSummary mode="refund" operation={resolvedOperation} />
        ...
        <section className="rounded-[28px] border border-white/70 bg-white p-6 shadow-card shadow-slate-900/5">
          ...
          <p className="mt-3 text-base leading-7 text-slate-600">
            Подтвердите возврат, если условия операции вас устраивают.
          </p>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            <SecondaryButton type="button" onClick={() => navigate(routes.refund(orderId))}>
              Отменить
            </SecondaryButton>
            {isFinal ? (
              <Link
